...
- `lib/src/core/rpc/subsonic_client.dart` —— Subsonic/OpenSubsonic 协议
  （Navidrome、Airsonic 等，`docs/SUBSONIC.md`），token 鉴权
  `t = md5(password + salt)`。
...
用户点击曲目
  → PlayerController.playQueue(tracks, index)
     → 重建队列(_rebuildQueue) → _startCurrent()
        → AudioEngine.load(track) → AudioEngine.play()
           → 引擎 statusStream/positionStream 回灌 PlayerController
              → notifyListeners()
                 → NowPlayingBar / PlayerPage / LyricsView 重绘
        → refreshLyrics() 读内联/本地 .lrc → LrcParser → LyricsView 滚动高亮
  → bootstrap 的桥接把 status/track/position 转发到 PluginEventBus
     → 订阅事件的插件（统计脚本 / 桌面歌词）更新自身状态
```
