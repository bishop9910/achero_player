...
拉取曲目元数据并**流式播放**。协议基于
[JSON-RPC 2.0](https://www.jsonrpc.org/specification) over HTTP，
纯 Dart 实现（见 `lib/src/core/rpc/music_server_client.dart`），
无任何原生依赖，四平台通用。
...
!!! tip "建议"
    ...
    `music.streamUrl` 返回的 `url` 直接带上有效期内的签名令牌，播放器拿到即可用。
...
!!! note "说明"
    ...
    `music.listAlbums` / `music.listArtists` / `music.listSongs` 是
