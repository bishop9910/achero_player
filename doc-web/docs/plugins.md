...
!!! abstract "插件有两种形态"
    ...
    - **运行时脚本插件**（推荐，用于轻量逻辑）：**独立的 `.dart` 文件**放在插件
      目录即可加载，无需重新编译。见 [运行时脚本插件](runtime-plugins.md)。
    - **编译侧插件**（本文档）：继承 `AcheroPlugin` 的 Dart 类，随应用一起编译。
      适合需要 `http` / `crypto` / 动画等重能力的插件。

!!! warning "重要前提"

    Flutter 在移动端 / 桌面端使用 AOT 编译，**无法在运行时加载任意 Dart 代码**。
    因此编译侧插件是「编译进应用」的 Dart 类——你编写插件后在
    `lib/src/plugin_bootstrap.dart` 注册，随应用一起编译。这换来的是零沙箱、
    全能力、类型安全的扩展体验。
...
!!! note "返回类型"
    ...
    `builder` 返回的是 `List<Widget>`（而非单个 widget）。
...
=== "编译侧"
    ...
    - `lib/src/builtin_plugins/tags_plugin.dart`（标签持久化 + 页面）
    - `lib/src/builtin_plugins/aurora_plugin.dart`（自定义 UI）
    - `lib/src/builtin_plugins/desktop_lyrics_plugin.dart`（多窗口）

=== "运行时脚本"

    - `assets/plugins/statistics_plugin.dart`（事件 + 持久化 + 页面）
    - `assets/plugins/theme_presets_plugin.dart`（设置区块）

!!! tip "对照阅读"

    「播放统计」已从编译侧迁移为**运行时脚本插件**
...
- `desktop_lyrics_plugin.dart` —— 桌面歌词：独立置顶悬浮窗显示歌词（仅桌面端，
  见 [桌面歌词窗口](desktop-lyrics.md)）。
- `music_server_plugin.dart` —— 演示「外部数据源 → 曲库」：通过 RPC
  音乐服务器拉取曲目（协议见 [RPC 协议](rpc.md)），并演示 URL 流媒体来源
  `UrlTrackSource` 与内联歌词。
- `subsonic_plugin.dart` —— Subsonic/OpenSubsonic 源与缓存（见 [Subsonic 源](subsonic.md)）。

运行时脚本插件（`assets/plugins/`，见 [运行时脚本插件](runtime-plugins.md)）：
